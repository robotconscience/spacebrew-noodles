...
// Below is 99% Kyle McDonald's fantastic ofxFacetracker blink example
...
public:
	...
	ofFbo eyeLeftFbo, eyeRightFbo;
	ofPixels eyePixels;
	ofImage sobelImgLeft, sobelImgRight;
	cv::Mat grayFloat;
	cv::Mat sobelx, sobely;
	cv::Mat rowMean, gray, sobel;
    
    bool computeEye( ofFbo & eyeFbo, ofImage & sobelImg, ofPolyline & rowMeanLine, float & runningMean, Graph & rowGraft );
    
	ofPolyline rowMeanLineLeft, rowMeanLineRight;
	ofMesh leftRectImg, rightRectImg;

	float runningMeanLeft, runningMeanRight;
    ...
    vector<ofPolyline> leftContours, rightContours;
    bool bLeftState, bRightState;
};
